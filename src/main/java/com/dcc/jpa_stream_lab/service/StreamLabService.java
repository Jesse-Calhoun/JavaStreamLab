...
public class StreamLabService {
    ...
    public List<User> RProblemFour()
    {
        ...
        return users.findAll().stream().filter(user -> user.getRegistrationDate().before(date)).toList();
    }
    ...
    public List<User> RProblemFive()
    {
        ...
        calen.set(2016, 12, 31);
        Calendar calendar = Calendar.getInstance();
        calendar.set(2018,01,01);

        Date date_one = calen.getTime();
        Date date_two = calendar.getTime();

        return users.findAll().stream().filter(user -> user.getRegistrationDate().before(date_two) && user.getRegistrationDate().after(date_one)).toList();
    }
    ...
    public List<Product> RProblemSix()
    {
        ...
        User user_one = users.findAll().stream().filter(user -> user.getEmail().equals("afton@gmail.com")).findFirst().orElse(null);
        List <Product> products = user_one.getShoppingcartItems().stream().map(i -> i.getProduct()).toList();
